...
export default function FiniteTree() {
  ...
  async function fetchWishes() {
    ...
    if (data) {
      const slotMap = {};
      data.forEach(wish => { slotMap[`${wish.grid_row}-${wish.grid_col}`] = wish; });
  ...
  const renderTreeRows = () => {
    ...
    for (let r = 1; r < TREE_SIZE; r++) {
      ...
      for (let i = 0; i < slotsCount; i++) {
        ...
        if (fulfilledWish) {
          let colorClass = COLOR_MAP[fulfilledWish.color] || COLOR_MAP.green;
          
          if (!COLOR_MAP[fulfilledWish.color]) {
          ...
          <form onSubmit={handleSubmit} className="bg-gray-900 text-white p-6 rounded max-w-sm w-full shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input className="w-full bg-black border border-gray-800 p-3 text-sm focus:outline-none focus:border-green-500 transition-colors text-center text-green-100 placeholder-gray-700" placeholder="Anonymous" value={newAuthor} onChange={e => setNewAuthor(e.target.value)} maxLength={30} />
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase block mb-1 tracking-wider">Message</label>
                <textarea required className="w-full bg-black border border-gray-800 p-3 text-sm focus:outline-none focus:border-green-500 transition-colors min-h-25 text-green-100 placeholder-gray-700 resize-none" placeholder="Write your blessing..." value={newMessage} onChange={e => setNewMessage(e.target.value)} maxLength={150} />
              </div>
            </div>
